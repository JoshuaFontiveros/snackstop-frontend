...
const Category = () => {
  ...
  useEffect(() => {
    axios.get("https://snackstop-v2.herokuapp.com").then((res) => {
  ...
  const onCategoryDelete = (id) => {
    ...
    axios.delete(`https://snackstop-v2.herokuapp.com/${id}`).then((res) => {
      console.log(res);
  ...
  const categorySubmitHandler = (e) => {
    ...
    if (
      ...
      axios
        .post("https://snackstop-v2.herokuapp.com", newCategory)
        .then((res) => {
          if (!res.data.error) {
